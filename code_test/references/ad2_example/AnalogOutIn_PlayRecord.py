...
# prime the buffer with the first chunk of ad2_data
cBuffer = c_int(0)
...
# loop to send out and read in ad2_data chunks
while iRecord < length :
    ...
    # play, analog out ad2_data chunk
    if sts.value == DwfStateRunning.value and iPlay < length:  # running and more ad2_data to stream
    ...
    # record, analog in ad2_data chunk
    if sts.value == DwfStateRunning.value or sts.value == DwfStateDone.value : # recording or done
        ...
        if dataAvailable.value > 0 :
            ...
            dwf.FDwfAnalogInStatusData16(hdwf, c_int(0), byref(record1, 2 * iRecord), c_int(0),
                                         dataAvailable)  # get channel 1 ad2_data chunk, offset for 16bit ad2_data is 2*
            dwf.FDwfAnalogInStatusData16(hdwf, c_int(1), byref(record2, 2 * iRecord), c_int(0),
                                         dataAvailable)  # get channel 2 ad2_data chunk
