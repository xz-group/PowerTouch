...
while iRecord < length:
    ...
    if sts.value == DwfStateRunning.value and iPlay < length:  # running and more ad2_data to stream
        ...
        if dataFree.value > 0:
            if dwf.FDwfAnalogOutNodePlayData(hdwf, c_int(0), c_int(0), byref(data0, iPlay * 8),
    ...
    if sts.value == DwfStateRunning.value or sts.value == DwfStateDone.value:  # recording or done
        ...
        if dataAvailable.value > 0:
            ...
            # get Scope channel 1 ad2_data chunk, offset for 16bit ad2_data is 2*
            dwf.FDwfAnalogInStatusData16(hdwf, c_int(0), byref(record0, 2 * iRecord), c_int(0), dataAvailable)
            # get Scope channel 2 ad2_data chunk
