...
#                               cDQ quad  bits 8    ad2_data 0
dwf.FDwfDigitalSpiWrite32(hdwf, c_int(4), c_int(24), rgdwTX, c_int(
    len(rgdwTX)))  # write from array of 32bit(int) elements in quad mode 24bit ad2_data lengths
dwf.FDwfDigitalSpiSelect(hdwf, c_int(0), c_int(1))  # CS DIO-0 high

dwf.FDwfDigitalSpiSelect(hdwf, c_int(0), c_int(0))  # CS DIO-0 low
dwf.FDwfDigitalSpiWrite(hdwf, c_int(1), c_int(8), rgbCmd, c_int(len(rgbCmd)))  # write 2 bytes to MOSI
dwf.FDwfDigitalSpiRead(hdwf, c_int(4), c_int(8), rgbRX,
                       c_int(len(rgbRX)))  # read to array of 8bit(byte) elements in dual mode 8bit ad2_data lengths
dwf.FDwfDigitalSpiSelect(hdwf, c_int(0), c_int(1))  # CS DIO-0 high
