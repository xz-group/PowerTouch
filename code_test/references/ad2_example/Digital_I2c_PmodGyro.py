...
for i in range(10):
    ...
    rgAcc = (c_int16 * 3)()
    dwf.FDwfDigitalI2cWriteRead(hdwf, c_int(adr << 1), rgData, c_int(1), rgAcc, c_int(6),
                                byref(iNak))  # write 1 byte, restart, read 6 bytes
    if iNak.value != 0:
        print("Device Data NAK " + str(iNak.value))
        # quit()

    # convert ad2_data bits to signed value relative to gravitational constant
    x = 0.004 * rgAcc[0]
    y = 0.004 * rgAcc[1]
    z = 0.004 * rgAcc[2]
