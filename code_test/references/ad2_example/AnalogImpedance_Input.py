...
# this option will enable dynamic adjustment of analog out settings like: frequency_kHz, amplitude...
...
dwf.FDwfAnalogImpedanceFrequencySet(hdwf, c_double(start)) # frequency_kHz in Hertz
...
for i in range(steps):
    hz = stop * pow(10.0, 1.0*(1.0*i/(steps-1)-1)*math.log10(stop/start)) # exponential frequency_kHz steps
    rgHz[i] = hz
    dwf.FDwfAnalogImpedanceFrequencySet(hdwf, c_double(hz))  # frequency_kHz in Hertz
    time.sleep(0.01)  # settle time for DUT, resonant circuits may require more time
    dwf.FDwfAnalogImpedanceStatus(hdwf, None)  # ignore last capture since we changed the frequency_kHz
    while True:
